...
import NoStream from './NoStream.jpg';

const HlsPlayer = ({stream}) => {
    const src = `/api/video/stream/${stream}.m3u8`
    const playerRef = React.useRef();

    const [streamIsActive, setStreamIsActive] = useState(false);
    useEffect(() => {
        const id = setInterval(() => {
            axios.get(src) // проверяю плейлист
                .then(resp => {
                    axios.get(`/api/video/info/${stream}/`) // проверю инфо
                        .then(resp => {
                            if (!streamIsActive) {
                                setStreamIsActive(resp.data.isAlive === true && resp.data.chunkNumber !== 0);
                            }
    ...
    }, [streamIsActive, setStreamIsActive]);
